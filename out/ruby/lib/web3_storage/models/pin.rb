  ...
  class Pin
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Web3Storage::Pin` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Web3Storage::Pin`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'peer_name')
        self.peer_name = attributes[:'peer_name']
      end
    ...
    def valid?
      status_validator = EnumAttributeValidator.new('String', ["PinQueued", "Pinning", "Pinned"])
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["PinQueued", "Pinning", "Pinned"])
    ...
    def hash
      [peer_id, peer_name, region, status, updated].hash
    end
